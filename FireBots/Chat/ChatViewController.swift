...
//        chatroomRef = FBChatroom.getChatroomRef().child(chatroom.id)
...
//            chatroomRef.child(teacherIDs.contains(senderId) ? FBConstant.Chatroom.teacher : FBConstant.Chatroom.user).removeObserver(withHandle: refHandle)
//        }
//    }
//    
...
//        })
//        
//        chatroomRef.child(FBConstant.Chatroom.teacher).observeSingleEvent(of: .value, with: { (snapshot) in
//            if snapshot.childrenCount > 0 {
//                for chatroomTeacherSnapshot in snapshot.children {
//                    if let chatroomTeacherSnapshot = chatroomTeacherSnapshot as? DataSnapshot {
//                        if chatroomTeacherSnapshot.key != self.senderId {
//                            let chatroomTeacherBadge = chatroomTeacherSnapshot.value as! Int
//                            let chatroomTeacherBadgeUpdate = [chatroomTeacherSnapshot.key: chatroomTeacherBadge + 1]
//                            snapshot.ref.updateChildValues(chatroomTeacherBadgeUpdate)
//                            
//                            FBUser.getUserRef().child(chatroomTeacherSnapshot.key).child(FBConstant.User.badge).observeSingleEvent(of: .value, with: { (userBadgeSnapshot) in
//                                if let userBadgeSnapshotValue = userBadgeSnapshot.value as? [String: AnyObject] {
//                                    let messageBadge = userBadgeSnapshotValue[FBConstant.User.message] as! Int
//                                    let userBadgeUpdate = [FBConstant.User.message: messageBadge + 1]
//                                    userBadgeSnapshot.ref.updateChildValues(userBadgeUpdate)
...
//        updateThisUserBadgeRefHandle = chatroomRef.child(teacherIDs.contains(senderId) ? FBConstant.Chatroom.teacher : FBConstant.Chatroom.user).observe(.childChanged, with: { (snapshot) in
...
//                    self.chatroomRef.child(self.teacherIDs.contains(self.senderId) ? FBConstant.Chatroom.teacher : FBConstant.Chatroom.user).updateChildValues(chatroomUserBadgeUpdate)
//
...
//                                    if currentChild == 0 {
//                                        FBChatroom.updateChatroomUserRead(chatroomID: self.chatroom.id, messageID: messageSnapshot.key)
...
//                        FBChatroom.removeChatroomUserRead(chatroomID: self.chatroom.id)
//                    }
//                    self.finishReceivingMessage()
//                    self.collectionView.isHidden = false
//                    self.dismissIndicator(view: self.mainViewController.view)
...
//                            FBChatroom.updateChatroomUserRead(chatroomID: self.chatroom.id, messageID: snapshot.key)
//                            self.finishReceivingMessage()
//                        } else if let photoURL = messageData[FBConstant.Message.photoURL] as? String {
//                            if let mediaItem = PhotoMediaItem(maskAsOutgoing: senderID == self.senderId) {
//                                
...
//                if teacher.id == uid {
//                    if let avatarImage = teacher.avatarImage {
//                        return avatarImage
//                    }
//                    break
//                }
...
//                FBChatroom.removeMessage(chatroomID: self.chatroom.id, message: message)
...
//            for package in chatroom.activityPackages {
//                for bookingChildID in package.bookedChildIDs {
//                    bookingChildren.append(bookingChildID)
//                }
//            }
//            viewController.schoolName = chatroom.schoolName
//            viewController.teachers = teachers
//            viewController.members = members
//            viewController.bookingChildren = bookingChildren
//        }
//    }
//}
//
...
//            if message.senderId != senderId {
//                if teacherIDs.contains(message.senderId) {
//                    cell.messageBubbleTopLabel.textColor = Style.Color.blue
//                }
//                else {
...
//        UIMenuController.shared.menuItems = [UIMenuItem.init(title: NSLocalizedString("Delete", comment: ""), action: #selector(deleteMessage))]
...
//            if self.sender.type == FBConstant.User.UserType.school && self.sender.schoolID == chatroom.schoolID {
//                return true
//            }
//            else {
//                return false
